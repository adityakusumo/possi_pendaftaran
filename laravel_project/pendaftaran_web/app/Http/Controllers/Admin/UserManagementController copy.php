...
class UserManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // NEW: Fetch distinct combinations of JENIS and NAMAKOTA from MstClub for Domisili dropdown
        $domisiliOptions = MstClub::select('JENIS', 'NAMAKOTA')
            ->distinct()
            ->get()
            ->map(function ($item) {
                // Concatenate JENIS and NAMAKOTA for display and value
                return [
                    'display' => $item->JENIS . ' ' . $item->NAMAKOTA,
                    'value' => $item->JENIS . ' ' . $item->NAMAKOTA // Value will be "Kab Kediri" etc.
                ];
            })
            ->sortBy('display'); // Sort alphabetically by display name
        ...
        return view('settings', [
            ...
            'roles' => $roles,
            'clubs' => $clubs,
            'domisiliOptions' => $domisiliOptions,
    ...
    public function updateUser(Request $request, User $user)
    {
        ...
        $request->validate([
            ...
            // Validation for domisili_select
            'domisili_select' => ['nullable', 'string', 'max:255'],
        ]);
        ...
        // --- NEW: Handle Domisili Update ---
        if ($request->filled('domisili_select')) {
            $selectedDomisiliString = $request->input('domisili_select');
            // Split "Kab Kediri" into "Kab" and "Kediri"
            $parts = explode(' ', $selectedDomisiliString, 2);
            $jenis = $parts[0] ?? null;
            $namakota = $parts[1] ?? null;

            // Find the corresponding MstClub record based on JENIS and NAMAKOTA
            $matchingMstClub = MstClub::where('JENIS', $jenis)
                ->where('NAMAKOTA', $namakota)
                ->first();

            if ($matchingMstClub) {
                // Update user's domisili columns with data from MstClub
                $user->update([
                    'KDPROPDOM' => $matchingMstClub->KDPROP,
                    'NAMAPROPDOM' => $matchingMstClub->NAMAPROP,
                    'KDJENISDOM' => $matchingMstClub->KDJENIS,
                    'JENISDOM' => $matchingMstClub->JENIS,
                    'KDKOTADOM' => $matchingMstClub->KDKOTA,
                    'NAMAKOTADOM' => $matchingMstClub->NAMAKOTA,
                ]);
                Log::info("User {$user->id} domisili updated to {$matchingMstClub->JENIS} {$matchingMstClub->NAMAKOTA}.");
            } else {
                Log::warning("No MstClub record found for selected domisili: {$selectedDomisiliString} for user {$user->id}. Clearing user domisili fields.");
                // Clear domisili fields if no match found (or if the selected value is invalid/cleared)
                $user->update([
                    'KDPROPDOM' => null,
                    'NAMAPROPDOM' => null,
                    'KDJENISDOM' => null,
            ...
            // If domisili_select is empty or null (e.g., cleared by user)
            $user->update([
                'KDPROPDOM' => null,
                'NAMAPROPDOM' => null,
                'KDJENISDOM' => null,
            ...
            Log::info("User {$user->id} domisili details cleared.");
        }
