...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.confirmDeleteAtlet = function () {
        ...
        if (!noniasToDelete) {
            alert('Pilih atlet dari daftar NIAS yang ingin dihapus terlebih dahulu.');
            return; // Prevent deletion if no NONIAS is selected
        }
        ...
        if (confirm('Apakah Anda yakin ingin menghapus atlet ini? Ini akan menghapus data atlet dengan NONIAS: ' + noniasToDelete)) {
            ...
            if (deleteForm) {
                // Set the value of the hidden input in the delete form
                document.getElementById('nonias_to_delete_hidden').value = noniasToDelete;
...
$(document).ready(function () {
    ...
    // const selectedNiasExpiredHidden = $('#selected_nias_exp1009');
    const selectedNiasExpiredHidden = $('#selected_nias_expired'); // New line
    ...
    niasTableBody.on('click', '.selectable-row', function () {
        ...
        selectedNiasExpiredHidden.val(athleteDetails.EXPIRED || '');
